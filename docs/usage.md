...
| 需要定义结构体 | 是 | 是 |
| 编译时类型安全 | 是 | 是 |
| 性能 | 高（直接 struct <-> wire bytes） | 高（编译时生成编解码逻辑） |
| 适用场景 | 已有 serde 生态的项目、需要共享模式 | Rust 端自定义协议 |
...
- **Go codec 方式** (手写 `MarshalSproto()/UnmarshalSproto()`): 每个结构体手动实现编解码方法，是最高性能方式

Rust 的 Serde 路径 (`to_bytes`/`from_bytes`) 在架构上与 Go reflect 方式对应。Derive 宏在编译时生成编解码逻辑，无需运行时模式查找。
